...
const formSchema = z.object({
  // reminder: don't forger to translate these messages and link them w/ back errcodes
  email: z
    .string()
    .min(4, {
...
export default function AuthFormSignUp() {
  ...
  async function handleHomeSignUp(values: z.infer<typeof formSchema>) {
    try {
      const response = await post<token>("/auth/register", {
      ...
      console.log("Sign Up Succeed!", response.data);
  ...
  async function handleSpotifySignUp(
    event: React.MouseEvent<HTMLButtonElement>,
  ) {
    handleOAuth2("spotify");
  }

  async function handleDiscordSignUp(
    event: React.MouseEvent<HTMLButtonElement>,
  ) {
    handleOAuth2("discord");
  }
  ...
  async function handleGitLabSignUp(
    event: React.MouseEvent<HTMLButtonElement>,
  ) {
    handleOAuth2("gitlab");
  }
  ...
  return (
    <div className="space-y-5 items-center">
      <Form {...form}>
        <form
          onSubmit={form.handleSubmit(handleHomeSignUp)}
          className="space-y-5"
        >
          <FormField
      ...
      <RoundedButton
        className="h-20 w-full text-3xl bg-discord_purple border-2 border-black"
