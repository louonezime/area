...
function ConnectReactionContent() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const serviceName = searchParams.get("serviceName");
  const serviceColor = searchParams.get("serviceColor");
  const reactionTitle = searchParams.get("reactionTitle");
  const reactionName = searchParams.get("reactionName");
  const reactionDescription = searchParams.get("reactionDescription");
  ...
  const [reactionFormInput, setReactionFormInput] = useState<ActionData | null>(
    null,
  );
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (popup && popup.closed) {
        ...
        try {
          if (
            popup.location.href.includes("http://localhost:8081/create/review")
          ) {
            ...
            clearInterval(interval);
            router.push(
              `/create/review?provider=${serviceName}?code=${code[1]}`,
            );
          }
        } catch (error) {
          console.error(error);
  ...
  const saveFormInput = (info: ActionData) => {
    setReactionFormInput(info);
    localStorage.setItem("reactionFormInput", JSON.stringify(info));
  };
    ...
    <>
      ...
      >
        ...
        <div className="text-white text-center text-2xl mt-[-25%] mb-[-15%] absolute overflow-hidden">
          {reactionTitle || "Unavailable reaction data."}
          <br />
          {reactionDescription || "Unavailable service description."}
          <form className="mt-7">
        ...
        <div className="absolute mt-[-655px]">
          <Link href="/create/reaction/choose/">
            <RoundedButton className="ml-[-700px] px-6 py-7 justify-center text-2xl bg-white text-black border-[5px] border-black overflow-hidden">
              <ChevronLeft /> Back
            </RoundedButton>
          </Link>
        </div>
