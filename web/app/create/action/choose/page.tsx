...
async function getServicesListAction(): Promise<Services[]> {
  try {
    const response = await get<ServiceResponse[]>(
      `/service/list?redirect=http://localhost:8081/create/reaction`,
    );
    const servicesData: Services[] = response.data
      ...
      .map((service, index) => ({
        ...
        actionsTitle: service.actions.map((action) => action.title),
        actionsDescription: service.actions.map((action) => action.description),
      }));
    return servicesData;
  } catch (error) {
  ...
  return (
    <>
      ...
      <div className="justify-center ml-[2%]">
        <GridServices data={getServicesListAction} />
      </div>
    </>
  );
}
